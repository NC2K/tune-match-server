  ...
  try {
    const data = await client.query(`
      SELECT  s.id, cat1, cat2, cat3, total, 
              u_name AS "uName", user_id AS "userId"
      FROM    scores s
      JOIN    users u
      ON      s.user_id = u.id;
    `);
...
app.put('/api/scores/:id', async (req, res) => {
  try {
    ...
    const data = await client.query(`
      UPDATE    scores
      SET       cat1 = $1, cat2 = $2, cat3 = $3, total = $4, u_name = $5
      WHERE     id = $6
      AND       user_id = $7
      RETURNING id, cat1, cat2, cat3, total, u_name AS "uName", user_id AS "userId";
    `, [score.cat1, score.cat2, score.cat3, score.total, score.uName, req.params.id, req.userId]);
